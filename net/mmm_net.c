...
static void process_player_sync(const uint8_t* payload, int len)
{
     ...
     * (see mserver.py build_player_sync comment). The speed field is
     * still decoded into rs->speed for ABI stability with any client
     * that might still read it, but the gameplay-loop consumer no
     * longer applies it to `players[op].physics_speed` (the actual
     * iter-4 user-facing fix). */
    if (len < 17) return;
