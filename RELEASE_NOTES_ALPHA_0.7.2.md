...
**3. PLAYER_SYNC wire-format — REVERTED back to iter-3 (17B payload, 19B wire)** — after auditing the full commit history I caught that a wire-format trim creates cross-version mismatch regressions (iter-3 client + iter-4 server, or vice versa, both produce WORSE freezing than the original bug — client rejects all syncs or decodes wrong offsets). Per the discipline established in `git log dd58d27`: don't introduce protocol changes that can produce silent decode misalignment if either side flashes a mismatched binary. The user's reported 3+P freeze is data-supported by the speed-override fix alone (#1); the wire trim was speculative bandwidth-pressure mitigation and not strictly needed.
